...
def _process_pattern_subset(
    ...
    try:
        ...
        answer = {}
        for pattern, pattern_id in grouped_patterns:
            ...
            this_step_elapsed = time.time() - this_step_computation_start_ts
            print(
                f"[worker {worker_id}][pid {os.getpid()}] finished processing pattern {pattern_id} in\n"
                f"\tcomputation overhead: {this_step_elapsed:.6f}",
                flush=True
            )
...
if __name__ == "__main__":
    ...
    with open(json_filename) as json_fin:
        ...
        n_processes = config["parallel"]["max_parallel_workers_per_gather"]
        ...
        with open(path_to_sql, "r") as fin, \
            ...
            conn, cursor = get_connection(dbname=dbname, autocommit=True) # this conn/cursor is just for getting the metadata and turning on/ off extensions, it won't be used for query processing, so it is in autocommit mode.
            workload_summary.get_column_metadata(
                cursor=cursor,
                alias_to_tab=alias_to_tab
            )
            turn_on_extensions(cursor)
            computation_start_ts = time.time()

            all_grouped_patterns = list(workload_summary.iterate_by_group())
            answer = {}

            n_workers = min(n_processes, len(all_grouped_patterns))
            pattern_subsets = [[] for _ in range(n_workers)]
            for i, grouped_pattern in enumerate(all_grouped_patterns):
                pattern_subsets[i % n_workers].append(grouped_pattern)
            ...
            for worker_id in range(n_workers):
                p = ctx.Process(
                    target=_process_pattern_subset,
                    args=(
                        worker_id,
                        pattern_subsets[worker_id],
                        workload_summary,
                        alias_to_tab,
                        dbname,
                        batch_size,
            ...
            for _ in range(n_workers):
                ...
                for qid, cnt in result["answer"].items():
                    if qid in answer:
                        errors.append(
                            f"duplicated qid {qid} from worker "
                            f"{result['worker_id']}"
                        )
                        continue
                    answer[qid] = cnt
            ...
            for p in processes:
                ...
                if p.exitcode != 0:
                    errors.append(
                        f"worker process pid={p.pid} exits with code {p.exitcode}"
                    )
            ...
            print(f"total computation overhead: {time.time() - computation_start_ts:.6f}")
