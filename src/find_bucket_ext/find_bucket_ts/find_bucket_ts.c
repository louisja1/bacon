...
PG_FUNCTION_INFO_V1(find_bucket_ts);

Datum find_bucket_ts(PG_FUNCTION_ARGS)
{
    ...
    TimestampTz val = PG_GETARG_TIMESTAMP(0);
    ...
    if (cache == NULL)
    {
        ...
        cache->data = (TimestampTz *)MemoryContextAlloc(
            fcinfo->flinfo->fn_mcxt,
            sizeof(TimestampTz) * dims[0] * dims[1]);
        memcpy(cache->data, ARR_DATA_PTR(buckets), sizeof(TimestampTz) * dims[0] * dims[1]);
        fcinfo->flinfo->fn_extra = cache;
    }

    TimestampTz *data = cache->data;
